  ...
  const applyAutoExposureHedges = (): void => {
    ...
    try {
      ...
      exposures.forEach((ex) => {
        ...
        // Avoid double counting multi-leg strategies (e.g. collar call+put).
        // For a given hedgeRequest, exposure coverage is best represented by the max leg notional, not the sum.
        const maxNotionalByRequest = new Map<string, number>();
        for (const inst of insts) {
          ...
          const n = Math.abs(Number((inst as any).notional || 0));
          if (!Number.isFinite(n) || n <= 0) continue;
          const prev = maxNotionalByRequest.get(rid) || 0;
          if (n > prev) maxNotionalByRequest.set(rid, n);
        }
        const coverage = Array.from(maxNotionalByRequest.values()).reduce((s, v) => s + v, 0);
        hedgedAbsByExposure.set(exId, coverage);
      });
      ...
      const updated = exposures.map((ex) => {
        ...
        // Allow >100% (over-hedging) but keep it finite.
        const ratio = amountAbs > 0 ? (hedgedAbs / amountAbs) * 100 : 0;
        ...
        Object.entries(currencyGroups).forEach(([currency, instrumentGroup]) => {
          ...
          Object.entries(maturityGroups).forEach(([maturityStr, maturityInstruments]) => {
            ...
            if (!existingExposure) {
              ...
              // Never modify a user-entered exposure's initial amount/type.
              // Only keep auto-generated exposures in sync; for manual ones, only update hedge fields.
              const isAutoGenerated =
                String((existingExposure as any).subsidiary || '') === 'Auto-Generated' ||
                String((existingExposure as any).description || '').toLowerCase().includes('auto-generated') ||
                String((existingExposure as any).description || '').toLowerCase().includes('auto-updated');

              const updatedExposure = isAutoGenerated
                ? {
                    amount: exposureAmount,
                    type: exposureType,
              ...
              const updateSuccess = serviceRef.current.updateExposure(existingExposure.id, updatedExposure as any);
              if (updateSuccess) {
                expositionsUpdated++; // ✅ COMPTEUR SÉPARÉ pour les mises à jour
                console.log(
                  `Updated existing exposure for ${currency} (${maturityStr}): ` +
                    (isAutoGenerated ? `amount=${exposureAmount}` : `hedge fields only`) +
                    ` - Hedge Ratio: ${maxHedgeQuantity}%`
                );
              }
            }
          });
